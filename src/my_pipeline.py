...
def pipeline(argv):
    ...
    ###############################################################
    #                TO RUN LARGE SAMPLE OF STARS                 #
    ###############################################################
    # Select a fraction of stars given by n-run.
    # The idea is to divide the full sample so that there are no more than 25 or 35 runs in total
    n_run = int(argv.n_run)
    l_files = len(input_files)

     # If number of files change group size to 35 instead of 25
    if l_files > 7000 and l_files < 10000:
        ngroups = 40
    elif l_files >= 10000:
        ngroups = 70
    else:
    ...
    if l_files < ngroups:
        ...
        print('\n------ Less files than required for sectioning sample ------')

    else:
        fraction = l_files // ngroups

        start = (n_run * fraction)
        end = ((n_run + 1) * fraction)
    ...
    resolved_modesfile_pdsfile = settings['pipeline'][11]['filenames']['resolved_modes']
    mle_resolved_modesfile_pdsfile = settings['pipeline'][11]['filenames']['mle_resolved_modes']
    mixed_modesfile_pdsfile = settings['pipeline'][11]['filenames']['mixed_modes']
    mle_mixed_modesfile_pdsfile = settings['pipeline'][11]['filenames']['mle_mixed_modes']
    final_modesfile_pdsfile = settings['pipeline'][11]['filenames']['final_modes']
    ...
    if not argv.start_function:
        path_to_file = 'stars_{}.csv'.format(n_run)

    else:
        if argv.start_function == "background":
            path_to_file = 'stars_background_{}.csv'.format(n_run)
        if argv.start_function == "resolved_modes":
            path_to_file = 'stars_resolved_modes_{}.csv'.format(n_run)
        if argv.start_function == "unresolved_modes":
            path_to_file = 'stars_unresolved_modes_{}.csv'.format(n_run)
        if argv.start_function == "final_fit":
            path_to_file = 'stars_final_fit_{}.csv'.format(n_run)
    ...
    print('Starts already done: ', stars_done, '\n')
    ...
    for input_file in input_files:
        ...
        if input_name in stars_done:
          ...
          count += 1
            ...
            print('.............................')
            print(' NEW STAR: ', input_name, '  ({}/{})'.format(count+1, len(input_files)))
            print('.............................')
            ...
            if not argv.start_function:
                ...
                if input_file not in Path(argv.output_directory, input_name).iterdir():
                    shutil.copyfile(input_file, Path(argv.output_directory, input_name, '{}.dat'.format(input_name)))


            with open(path, mode = 'a') as fstar:
                writer = csv.writer(fstar, delimiter = ',')
                #set all flags to -1
                flag_cv = -1
                flag_numax = -1
                ...
                if not argv.start_function:
                    ...
                    if cv_method:
                        ###################################################
                        #          IDENTIFICATION OF POWER EXCESS         #
                        ###################################################
                ...
                if (flag_cv < 2)  and (flag_numax <= 1):
                    if not argv.start_function or argv.start_function == "background":
                        ...
                        path_bgr_pds = Path(argv.output_directory, input_name, background_corrected_pdsfile)
                        path_bgr_over_pds = Path(argv.output_directory, input_name, background_corrected_oversampled_pdsfile)

                        if not path_bgr_pds.is_file() or not path_bgr_over_pds.is_file():
                            print('Necessary files not available for peak find, star not analysed')
                            flag_bgr = 1
                    ...
                    if flag_bgr <= 0:
                        if not argv.start_function or argv.start_function == "background" or argv.start_function == "resolved_modes":
                            ...
                            peaks.to_csv(Path(argv.output_directory, input_name, resolved_modesfile_pdsfile), index = False)
                            ...
                            if (len(peaks.frequency)) >= 1:
                                ...
                                summary.to_csv(Path(argv.output_directory, input_name, summaryfile), index = False)
                                peaks_mle.to_csv(Path(argv.output_directory, input_name, mle_resolved_modesfile_pdsfile), index = False)
                            ...
                            peaks = pd.read_csv(Path(argv.output_directory, input_name, resolved_modesfile_pdsfile), delimiter = ',')
                            if (len(peaks.frequency)) >= 1:
                                flag_mle_resolved = 0
                                peaks_mle = pd.read_csv(Path(argv.output_directory, input_name, mle_resolved_modesfile_pdsfile), delimiter = ',')
                                if 'l' in peaks_mle:
                                    flag_02 = 0
                                else:
                                    flag_02 = 1
                        ...
                        if flag_02 == 0.0:
                            if not argv.start_function or argv.start_function == "background" or argv.start_function == "resolved_modes" or argv.start_function == "unresolved_modes":
                                ...
                                summary.to_csv(Path(argv.output_directory, input_name, summaryfile), index = False)
                                mixed_peaks.to_csv(Path(argv.output_directory, input_name, mixed_modesfile_pdsfile), index = False)
                                ...
                                summary.to_csv(Path(argv.output_directory, input_name, summaryfile), index = False)
                                mixed_peaks.to_csv(Path(argv.output_directory, input_name, mle_mixed_modesfile_pdsfile), index = False)
                            else:
                                mixed_peaks = pd.read_csv(Path(argv.output_directory, input_name, mle_mixed_modesfile_pdsfile), delimiter = ',')
                            ...
                            if (flag_mle_mixed == 0):
                                if not argv.start_function or argv.start_function == "background" or argv.start_function == "resolved_modes" or argv.start_function == "unresolved_modes" or argv.start_function == "final_fit":
                                    ...
                                    summary.to_csv(Path(argv.output_directory, input_name, summaryfile), index = False)
                                    all_peaks.to_csv(Path(argv.output_directory, input_name, final_modesfile_pdsfile), index = False)
                ...
                # Update a file with flag information per star
                with open(Path(argv.output_directory, 'data_sample_{}.csv'.format(n_run)), mode = 'a+') as fdata_file:
                    writer2 = csv.writer(fdata_file, delimiter = ',')

                    # To check if file is new or old. If new, write column names
                    fdata_file.seek(0)
                    lines = fdata_file.readlines()
                    if len(lines) == 0:
                        writer2.writerow(summary.columns)

                    if summary is not None:
                        writer2.writerow(summary.iloc[0])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--quiet', '-q', action='store_true',
                        help="No output")
    parser.add_argument('--n-run', '-n', default = 0)
