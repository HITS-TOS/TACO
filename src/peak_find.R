...
peak_find_r <- function(pds, ofac_pds, data, peaks, snr, prob,
    ...
    if (removel02 == TRUE) {
        ...
        if (is.null(maxlwd) || is.na(maxlwd)) {
            ...
            if (maxlwd < 5 * deltanu / 2) {
                print("Maximum peak linewidth (HWHM) is less than 5 times frequency resolution! Increasing upper bound slightly")
                #maxlwd <- deltanu / 2 + 0.1 * deltanu / 2
                #make it at least 10 points
                maxlwd <- 10 * deltanu / 2
            }
