...
p <- add_argument(p, arg = "--pds", nargs=1, default = "pds_bgr.csv",
                  help = "File name of the csv file with the PDS It must contain the columns 'frequency' and 'power",
                  type = "character", flag = FALSE)
p <- add_argument(p, arg = "--plot", nargs=1, default=FALSE,
                  help = "Whether or not plot tagging runs, this is useful for debugging. Defaults to FALSE",
                  type = "character", flag = FALSE)
argv <- parse_args(p)
...
if(!file.exists(argv$pds))
    stop(paste("Could not find the file with the PDS:", argv$pds))
if(!file.exists(file.path(script.basename, "peakFind_lib.R")))
    stop("Could not find peakFind_lib.R")
if(!file.exists(file.path(script.basename, "l02_modes_id.R")))
    stop("Could not find l02_modes_id.R")
